...
import os, platform, requests, re, time, base64, shutil, traceback, datetime, multiprocessing
import pandas as pd
...
from PyQt6.QtCore import Qt, QDir, QCoreApplication, QObject, QRunnable, pyqtSlot, QThreadPool#, QTimer, QUrl
...
'''
    ...
    def download_audio(self):
        ...
        # self.quitBtn.hide()	                      # Need GUI to be updated during download process
        # self.button.hide()	                      #
        # self.urlBox.hide()                          #
        ###############################################
        self.downloadBtn.setEnabled(False)
        ...
        ## BUG ######################################
        ## Updates GUI with data regarding the current download process
        ...
        with open(self.links_file) as linksText:
            for l_no, line in enumerate(linksText):
                ...
                ## BUG ######################################
                ## Send percentage value to progress bar
                ...
                if self.option_overwrite.isChecked():
                    ## BUG ######################################
                    ## Displays message confirming overwrite option
                    # if count < 1:
                    ...
                    count += 1
                    ## BUG ######################################
                    ## Resets GUI back to original state
                    # else:
                    #     self.reset_gui()
                    #     return
                    ###############################################
        ...
        if count_length == self.episode_count:
            ...
            ## BUG ######################################
            ## Opens a prompt displaying download location
            # self.download_complete(show_title, show_dir)
            # self.reset_gui()
            ###############################################
    ...
    def quit(self):
        ...
        ## BUG ######################################
        ## Exits app
        # sys.exit() ## CAN'T RUN WHILE DOWNLOADING
        ###############################################
